...
description: 在 macOS 安裝 / 設定 / 移除 Claude Code 的「純音效」hook。讓 Claude 在「每次跑 bash」「換你了（這輪結束）」「需要你回應（權限等待）」三個時機播放系統音效，不跳任何桌面通知。觸發詞：「裝 Claude 音效」「設定 Claude 音效」「claude sounds」「移除 Claude 音效」。僅支援 macOS，需要 jq。
---
...
- `NOTIFY_BASH_*` / `NOTIFY_DONE_*` / `NOTIFY_WAITING_*`：各自的 `ENABLED`（開關）、`SOUND`
...
只移除本工具自己的三個 hook（保留其餘 settings），並刪除 skill symlink 目錄。設定檔預設保留。

## 注意事項

- **僅 macOS**（用系統內建 `afplay` 播 `/System/Library/Sounds`）。
- 三個 hook 都「**同步**」播放，**不可掛 async**：async 會在 `afplay` 播完前把行程收掉 → 聽不到聲音。
