...
description: 在 macOS 安裝 / 設定 / 移除 Claude Code 的「純音效」hook。讓 Claude 在「每次跑 bash」「回答完畢」「需要你回應（權限等待）」三個時機播放系統音效，不跳任何桌面通知。觸發詞：「裝 Claude 音效」「設定 Claude 音效」「claude sounds」「移除 Claude 音效」。僅支援 macOS，需要 jq。
---
